...
async def handle_book_string(update: Update, scheduler: Scheduler, book_string: str):
    ...
    try:
        ...
        if (booking_time - datetime.now()).total_seconds() <= 0:
            await update.message.reply_text("Booking time has already passed! Cannot schedule booking.")
            return f"Handled the book string: {book_string}"
        
        booking_job = Job(
            job_enum=JobEnum.BookSession,
            time=job_time,
            data={"chat_id": update.effective_chat.id, "session_id": session_id, "booking_time": booking_time}
        )
    ...
    # For demonstration, let's just print the string back
    return f"Handled the book string: {book_string}"